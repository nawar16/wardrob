      ...
      <form method="post" action="{{route('product.update',$product->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="condition" class="form-control">
              ...
              <option value="default" {{(($shop->products()->where('product_id',$product->id)->first()->pivot->condition=='default')? 'selected':'')}}>Default</option>
              <option value="new" {{(($shop->products()->where('product_id',$product->id)->first()->pivot->condition=='new')? 'selected':'')}}>New</option>
              <option value="hot" {{(($shop->products()->where('product_id',$product->id)->first()->pivot->condition=='hot')? 'selected':'')}}>Hot</option>
          </select>
        </div>

        <div class="form-group">
          <label for="stock">Quantity <span class="text-danger">*</span></label>
          <input id="quantity" type="number" name="stock" min="0" placeholder="Enter quantity"  value="{{$shop->products()->where('product_id',$product->id)->first()->pivot->stock}}" class="form-control">
          @error('stock')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
