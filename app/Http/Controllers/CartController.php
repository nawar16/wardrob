...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        if (empty($request->shop)) {
            request()->session()->flash('error','Please select a single shop');
            return back();
        }  
        $product = Product::where('slug', $request->slug)->first();
        $shop = Shop::where('id', intval($request->shop))->with(['products'])->first(); 
        $stock=0;
        foreach($shop->product_by_id($product->id)->shops as $p)
        {
            $pivot = $p->pivot;
        ...
        $res = $rec->add_purchase(auth()->user()->name, $product->title);
